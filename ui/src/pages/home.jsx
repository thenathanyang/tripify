      ...
      <div>
        ...
        <div className="container">
          ...
          <Section title={this.getUpcomingTitle()}>
            { upcomingTrips.map(trip => <Link to={`/trips/${trip.id}`}><TripTile key={trip.id} title={trip.name} background={trip.background} /></Link>) }
          </Section>
          { pastTrips.length > 0 &&
            <Section title="Past">
              { pastTrips.map(trip => <Link to={`/trips/${trip.id}`}><TripTile key={trip.id} title={trip.name} background={trip.background} /></Link>) }
            </Section>
          }
        </div>
      </div>
